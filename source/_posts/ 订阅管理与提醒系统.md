...
description: null
excerpt: SubsTracker – 订阅管理与提醒系统是基于Cloudflare Workers的轻量级订阅管理系统，帮助您轻松跟踪各类订阅服务的到期时间，并通过Telegram发送及时提醒。   ✨ 特性   🔔 自动提醒: 在订阅到期前自动发送Telegram通知   📊 订阅管理: 直观的Web界面管理所有订阅   🔄 周期计算: 智能计算循环订阅的下一个周期   📱 响应式设计: 完美适配...
...
updated: '2025-07-15T17:21:51.619+08:00'
---
**SubsTracker – 订阅管理与提醒系统是**基于Cloudflare Workers的轻量级订阅管理系统，帮助您轻松跟踪各类订阅服务的到期时间，并通过Telegram发送及时提醒。
...
* 🔒 **安全可靠**: 数据存储在Cloudflare KV中，安全且高效
  ...
  ** V4 版本代码：**
  ...
  SubsTracker 开源项目【[仓库]([https://www.freedidi.com/?golink=aHR0cHM6Ly9naXRjb2RlLmNvbS9naF9taXJyb3JzL3N1Yi9TdWJzVHJhY2tlcg==](https://github.com/wangwangit/SubsTracker))】
* ## 如何获取[Telegram](https://www.freedidi.com/?golink=aHR0cHM6Ly90ZWxlZ3JhbS5vcmcv)的`Bot_Token`和`Chat_ID`

  如果您还没有Telegram账户，请先创建一个。接着，按照以下步骤操作以获取`BOT_TOKEN`和`CHAT_ID`：


  1. **获取`Bot_Token`**
     * 在Telegram中，向[@BotFather](https://www.freedidi.com/?golink=aHR0cHM6Ly90Lm1lL0JvdEZhdGhlcg==)发送命令`/newbot`，根据提示依次输入您的机器人名称和用户名。成功创建机器人后，您将会收到一个`BOT_TOKEN`，用于与Telegram API进行交互。（这一串api就是Bot_Token)
...
2：接着搜索：@getmyid\_bot 这个机器人并发送信息/start 给它就可以获得你的 Chat ID（Current chat ID）
...
**注意！！**：在开始之前，需在Cloudfalre控制左侧面板里，依次点击：存储和数据库 – KV – 创建 – 创建KV空间，空间名称填写：SUBSCRIPTIONS\_KV 保存。**否则 SubsTracker 添加订阅将无法保存！**

1.登陆cloudflare,创建worker,粘贴本项目中的js代码,点击部署

<img src="https://5f4480c.webp.li/2025/07/f3cacad2c75008b233493d4c9b66f9bf.png" >

2.创建KV键值 **SUBSCRIPTIONS\_KV**
